...
#define SI_XTAL_FREQ	25001800UL	// Replace with measured crystal frequency of XTAL for CL = 10pF (default)
...
/* I2C1 pins */
#define I2C1_SDA 14 //was originally 18
#define I2C1_SCL 15 //was originally 19
...
void si_evaluate(void)
{
	...
	if (vfo[0].flag)
	{
		msn = (float)(vfo[0].msi * vfo[0].ri * vfo[0].freq) / SI_XTAL_FREQ;
	...
	if (vfo[1].flag)
	{
		vfo[1].ri  = (vfo[1].freq<1000000)?128:((vfo[1].freq<3000000)?32:1);
		//vfo[1].ri  = ((vfo[1].freq<3000000)?32:1);
		//vfo[1].ri = 1;		// Pre-scale Ri, stretch down Ri=1 range
		if ((vfo[1].freq >= 3000000)&&(vfo[1].freq < 6000000))						// Low end of Ri=1 range
			vfo[1].msi = (uint8_t)126;												// Maximum MSi on Fvco=(4x126)MHz
		else
			vfo[1].msi = (uint8_t)(750000000UL / (vfo[1].freq * vfo[1].ri)) & 0xfe;	// Calculate MSi on Fvco=750MHz
		msn = (float)(vfo[1].msi * vfo[1].ri * vfo[1].freq) / SI_XTAL_FREQ;
		vfo[1].msn = msn;
		vfo[1].ri *= 4; // AA1GD set output divider to 4 times original, so can generate the .25hz increments
		//needed for FT8
